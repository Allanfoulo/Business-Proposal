...
def add_markdown_table(doc, text):
    """Parse and add markdown tables to document (FIXED: defines num_rows, num_cols)"""
...
def collect_basic_info():
    ...
    if st.button('Submit'):
        ...
        # Create section to header mapping (SECURITY FIX: removed eval())
        section_to_header = {
        ...
        # COMPLETED: Populate PDF with sections
        ...
        # Add sections (SECURITY FIX: using section_to_header dict instead of eval())
        for section in sections:
            header_text = section_to_header.get(section, "Section")
            ...
            # OPTIMIZED: Use helper function instead of inefficient parts.index()
            p = doc.add_paragraph()
            ...
            # FIXED: Check if the section contains tables (now properly defines num_rows/num_cols)
            if "|" in section:
